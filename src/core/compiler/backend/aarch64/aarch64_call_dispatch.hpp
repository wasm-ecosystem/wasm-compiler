///
/// @file aarch64_call_dispatch.hpp
...
#ifndef AARCH64_CALL_DISPATCH_HPP
#define AARCH64_CALL_DISPATCH_HPP
...
namespace vb {
namespace aarch64 {
/// @brief Base class for function call dispatch on AArch64
class CallBase {
protected:
  ...
  CallBase(AArch64_Backend &backend, uint32_t const sigIndex, uint32_t const stackParamWidth, uint32_t const stackReturnWidth) VB_THROW
      : backend_(backend),
        sigIndex_(sigIndex),
...
public:
  /// @brief Store link register to stack
  inline void storeLR() {
    static_assert((ImplementationLimits::numParams * 8U) <= (0xFFFU * 8U), "Too many arguments");
    backend_.as_.INSTR(STR_xT_deref_xN_imm12zxls3_t).setT(REG::LR).setN(REG::SP).setImm12zxls3(SafeUInt<15U>::fromUnsafe(of_lr_))();
  }
  /// @brief Restore link register from stack
  inline void restoreLR() {
    backend_.as_.INSTR(LDR_xT_deref_xN_imm12zxls3_t).setT(REG::LR).setN(REG::SP).setImm12zxls3(SafeUInt<15U>::fromUnsafe(of_lr_))();
  }
...
protected:
  AArch64_Backend &backend_;        ///< Reference to the AArch64 backend
...
private:
  ...
  uint32_t of_lr_;                           ///< Offset for link register
};
/// @brief Direct V2 import call handler for AArch64
...
public:
  ...
  /// @param backend Reference to the AArch64 backend
  /// @param sigIndex Function signature index
  DirectV2Import(AArch64_Backend &backend, uint32_t const sigIndex) VB_THROW
      : CallBase(backend, sigIndex, backend.moduleInfo_.getNumParamsForSignature(sigIndex) * 8U,
...
/// @brief Base class for V1 calling convention
...
public:
  ...
  /// @param backend Reference to the AArch64 backend
  /// @param sigIndex Function signature index
  ImportCallV1(AArch64_Backend &backend, uint32_t const sigIndex) VB_THROW
      : V1CallBase(backend, sigIndex, backend.getStackParamWidth(sigIndex, true)) {
  }
...
/// @brief Internal call handler for AArch64
// coverity[autosar_cpp14_m3_4_1_violation]
...
public:
  ...
  /// @param backend Reference to the AArch64 backend
  /// @param sigIndex Function signature index
  InternalCall(AArch64_Backend &backend, uint32_t const sigIndex) VB_THROW
      : V1CallBase(backend, sigIndex, backend.getStackParamWidth(sigIndex, false)) {
  }
